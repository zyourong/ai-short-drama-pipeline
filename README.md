...
| 指标 | 英文标识 | 预期标准（合格阈值） | 基准均值（全量实测） | 说明 |
|------|---------|-------------------|-------------------|------|
| BRISQUE画质评分 | `brisque_score` | **< 45** | 人物50.1 / 道具85.9 / 场景29.8 | 无参考画质评分，基于自然场景统计，越低越好 |
| 拉普拉斯清晰度 | `blur_score` | **> 50** | 人物559.9 / 道具63.1 / 场景84.1 | 拉普拉斯算子边缘方差，越高越清晰 |
| 画质综合判定 | `iqa_pass` | brisque<45 **且** blur>50 | — | 两项同时满足才判定画质合格 |
| CLIP图文匹配度 | `clip_score` | **≥ 0.26** | 人物0.306 / 道具0.318 / 场景0.290 | CLIP模型图像与提示词余弦相似度，越高匹配越好 |
...
| 指标 | 英文标识 | 预期标准（合格阈值） | 基准均值（全量实测） | 说明 |
|------|---------|-------------------|-------------------|------|
| 人体关键点数量 | `keypoint_count` | **= 17（必须完整）** | 17.0 | YOLOv8-pose检测COCO标准17关键点（鼻/眼/耳/肩/肘/腕/髋/膝/踝），少于17表示人物截断或缺失 |
| 人体完整性判定 | `integrity_pass` | keypoint_count == 17 | — | 人体完整合格判定，不完整触发自动返工 |
| 分区域CLIP一致性 | `intra_clip_consistency` | **≥ 0.70** | 0.763 | 1×4横排图裁4区域（特写/正视/侧视/后视），CLIP特征两两相似度均值，衡量多视角一致性 |
| 人脸一致性 | `intra_face_consistency` | **≥ 0.72** | 0.827 | InsightFace提取第1格（特写）与第2格（正视图）人脸特征余弦相似度，需手动下载buffalo_l模型 |

### 道具专属

| 指标 | 英文标识 | 预期标准（合格阈值） | 说明 |
|------|---------|-------------------|------|
| 是否检测到文字 | `has_text` | —（信息项） | OCR是否检测到文字，未检测到时所有文字校验跳过，不计入合格率分母 |
| 检测到的文字 | `detected_text` | —（信息项） | OCR识别的文字内容 |
| 文字语种校验 | `text_lang_pass` | 亚洲风格：中文字符占比 **≥ 30%**；欧美风格：英文字符占比 **≥ 50%** | 数字和符号不计入分母，OCR未检测到文字时跳过 |
| 关键词校验 | `text_keyword_pass` | 包含核心关键词即合格 | 无预期关键词时跳过（None） |
| 文字综合判定 | `text_pass` | text_lang_pass **且** keyword_pass | 文字综合合格，OCR未检测到文字时跳过 |

### 场景专属

| 指标 | 英文标识 | 预期标准（合格阈值） | 基准均值（全量实测） | 说明 |
|------|---------|-------------------|-------------------|------|
| 多视图CLIP一致性 | `group_consistency` | **≥ 0.65**（场景四视图） | — | 四宫格2×2裁4视图，CLIP特征两两相似度均值，衡量同一场景多视角统一性 |
| 色调一致性 | `scene_color_consistency` | **≥ 0.40** | — | HSV直方图相关系数，四视图内容差异大故阈值放宽（原0.70过严） |

> **已移除指标**：场景pHash差异（`phash_avg_diff`，原阈值≤15），经实测验证不适合场景四视图一致性评估（人眼判断一致但pHash差异大），已从质检流程中移除。

### 假设检验标准

| 项目 | 标准 |
|------|------|
| 检验方法 | 曼-惠特尼U检验（Mann-Whitney U Test），非参数双侧检验，不要求数据正态分布 |
| 显著性水平 | α = 0.05 |
| 合格判定 | p > 0.05：新批次与基准无显著差异，质量达标；p ≤ 0.05：存在显著差异，需关注 |
| 优于基准 | p ≤ 0.05 且新批次均值优于基准方向，判定为"显著优于基准" |
| 基准目录 | 默认 `RunningHub_Outputs/`，可在 `launcher.py` 中配置 `BASELINE_DIR` |
